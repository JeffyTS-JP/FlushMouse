...
* **2025/08/31　　Version 3.6.0.11 を リリースしました**

* **(主な変更点)**
   * CPU 使用率が下がるように調整しました (特に、直接描画方式と併用モードを使用する場合)
   * 上記修正では、マウスの動きをイベントドリブンを併用するようにして、ポーリングでの処理を減らしました。このためマイナーバージョンを上げました
   * キーリピート、Caps Lock の処理を改善しました
