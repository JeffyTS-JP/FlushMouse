...
* **2025/08/31　　Version 3.7.0.5 を リリースしました**

* **(主な変更点)**
   * Caps Lock が ON の時に、リソースで表示する部分もアンダーラインを表示するようにしました
   * CPU 使用率が下がるように調整しました (タイマーでの処理をなくしました)
   * Windows Remote Desktop と VMware guest のウィンドウ内で、できる限り Host の IME モードを表示しないように設定で選べるようにしました
