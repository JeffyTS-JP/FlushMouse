...
* **2024/11/03　　Version 3.3.0.15 を リリースしました**

* **(主な変更点)**
   * マウスカーソル右下の IME モード表示を直描画ではない時でも、表示文字、表示色、フォントを変更できるようにしました
   * (全ての) IME モード表示の描画方法、表示位置の処理を見直しました
   * IME モード表示設定画面を Expander対応にしました
...
#### マウスカーソル
   ...
   * キャレットとマウスカーソル左への IMEモード表示に、Caps Lock ONの時はアンダーラインを表示します (直接描画ではない場合のマウスカーソル右下の表示にはアンダーラインは表示されません。ご了承ください)
