...
* **2024/11/08　　Version 3.4.0.7 を リリースしました**

* **(主な変更点)**
   * 直接描画とリソースを使用しての描画を併用できるモードを新設しました
   * マウスカーソルがビジー表示の時(砂時計マークなど)では、IME モードの表示を行わないようにしました
   * (初回インストール時などの)設定のレジストリの無い状態で、設定を開いたとき、画面中央に表示しないバグを修正しました
   * IME モード表示設定で、文字入力の際、2文字しか受けつけないテキストボックスが存在したバグを修正しました
   * 設定の左ペインのデザインを修正しました
...
#### 対応OS
   * Windows 10 / 11 64bit  (Latest Release Versionで検証しています。24H2 については準備が整い次第確認します)
...
#### マウスカーソル
   ...
   * 「直接描画方式を使用する」と「リソース方式と直接描画方式を併用する」の場合はすべてのマウスカーソルで IME のモード表示が行えます(ただし、システムビジー(砂時計など)のカーソルでは表示しません)
...
|リソース方式と直接描画方式を併用する|矢印、Iビーム、ハンドをリソースで表示し、残りは直接描画します|ほとんどのケースでリソースで表示することになりますので遅れも目立たないと思います|2方式を併用してますのでのマウスサイズなどの設定が少々面倒ですが[こちら](https://github.com/JeffyTS-JP/FlushMouse?tab=readme-ov-file#%E8%A8%AD%E5%AE%9A%E3%81%AE%E3%83%92%E3%83%B3%E3%83%88)を参考にしてみてください|
...
<img src="https://github.com/JeffyTS-JP/FlushMouse/blob/master/OSDN%20Wiki/IMEModeDlg3.png" width="50%">
   ...
   * また、「IME モードを描画で表示する」場合、マウスカーソル右下の表示位置の距離を変更できます(正確にはホットスポットからの距離です)。お好みで設定してください
