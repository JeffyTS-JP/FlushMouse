...
* **2024/11/15　　Version 3.4.3.1 を リリースしました**

* **(主な変更点)**
   * 前バージョンで、画面がちらついてしまうため修正しました
   * 以下は Ver.3.4.2.4 の内容です
   * VisualStudio 2022 Ver.17.11.5 で動作しない問題は Ver.17.12.0 で解消されました
   * 「リソース方式と直接描画方式を併用する」の処理を見直して、CPU 使用率を下げました
   * 「直接描画方式」、「リソース方式と直接描画方式を併用する」で、サイズ変更にかかわるシステムで表示するカーソルには、IME モードを表示しないようにしました
...
#### マウスカーソル
   ...
   * 「直接描画方式を使用する」と「リソース方式と直接描画方式を併用する」の場合はすべてのマウスカーソルで IME のモード表示が行えます(ただし、システムで表示する、ビジー(砂時計など)や、サイズ変更にかかわるカーソルでは表示しません)
