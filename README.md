...
* **2026/04/12　　Version 4.0.1.8 を リリースしました**

* **(主な変更点)**
	* Microsoft.WindowsAppSDK を 2.0.1 にアップデートしました
	* 起動時のバグを修正しました
	* その他、細かなバグの修正や機能改善をしました
	* ***FlushMouse の Ver. 3.x.x.x からアップグレードする場合は、お手数ですが、コントロールパネルの「プログラムと機能」、または設定の「アプリ」「インストールされているアプリ」から、いったんアンインストールしてから、FlushMouse Ver. 4.x.x.x をインストールしてください***
