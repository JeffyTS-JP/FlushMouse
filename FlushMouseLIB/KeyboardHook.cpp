...
// bKeyboardHookLLSet()
//
BOOL	bKeyboardHookLLSet(HWND hWnd)
{
	if (!hWnd || hWndKBParentLL)	return FALSE;
	hWndKBParentLL = hWnd;
	hHookLL = SetWindowsHookEx(WH_KEYBOARD_LL, (HOOKPROC)lpKeyboardHookLLProc, GetModuleHandle(NULL), 0);
	if (!hHookLL) {
		if (GetLastError() == 0) {
			hHookLL = SetWindowsHookEx(WH_KEYBOARD_LL, (HOOKPROC)lpKeyboardHookLLProc, GetModuleHandle(NULL), 0);
			if (!hHookLL) {
...
//
BOOL	bSetEnableEPHelperLL64(BOOL bEPHelper)
{
...
//
BOOL	bSetEnableIMEModeForcedLL64(BOOL bIMEModeForced)
{
